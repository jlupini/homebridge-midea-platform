...
import { ParseMessageResult, ProtocolVersion, TCPMessageType } from './MideaConstants.js';
...
import { LocalSecurity, SIGN_MISMATCH_ERROR_MESSAGE } from './MideaSecurity.js';
...
export default class MideaDevice extends EventEmitter {
    ...
    fetch_v2_message(messageToProcess) {
        const result = [];
            ...
            while (!this.promiseSocket.destroyed) {
                try {
                    ...
                    if (e instanceof Error && e.message === SIGN_MISMATCH_ERROR_MESSAGE) {
                        this.logger.warn(`[${this.name} | run] Sign mismatch — V3 crypto state desynced. Resetting and forcing reconnect + re-handshake.`);
                        this.security.reset();
                        this.close_socket();
                        break;
                    }
